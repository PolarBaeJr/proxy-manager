...
func main() {
	...
	case peerSecret != "" && len(peerList) > 0:
		...
		log.Printf("dashboard peers: full mesh — handshaking with %d peer(s) every %s, /peer/handshake, /peer/service-status, /peer/services, /peer/services/, /peer/images, /peer/images/, /peer/access, /peer/logs/containers, and /peer/logs/ on %s %s", len(peerList), *peerSyncInterval, *peerAddr, writesMsg)
	case peerSecret != "":
		peerServer(*peerAddr, peerHandlers)
		log.Printf("dashboard peers: /peer/handshake, /peer/service-status, /peer/services, /peer/services/, /peer/images, /peer/images/, /peer/access, /peer/logs/containers, and /peer/logs/ enabled on %s (receive-only, no outbound peers configured) %s", *peerAddr, writesMsg)
	case len(peerList) > 0:
